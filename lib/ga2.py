...
from lib.neural_network2 import F2AN2
...
class Individual:
    ...
    def __init__(self, net_layers, weights=None, biases=None):
        self.brain = F2AN2(*net_layers)
        self.genes = dict()

        if np.any(biases):
            self.genes['biases'] = biases

        if np.any(weights):
            self.genes['weights'] = weights

        self.genes = self.brain.export_weights()
        self.score = 0
        self.build_brain()
        self.outputs = []
    ...
    def build_brain(self):
        self.brain.import_weights(self.genes)
...
class Generation:
    ...
    def initialize(self, population=None):
        if population:
            ...
            self.population = [Individual(net_layers=self.net_layers) for _ in range(self.pop_size)]
...
class GeneticAlgorithm:
    ...
    def load_many(self, filename):
        genes_list = np.load(filename, allow_pickle=True)
    ...
    def crossover(self, parents):
        point_w = random.randint(1, len(parents[0].genes['weights']) - 2)
        point_b = random.randint(1, len(parents[0].genes['biases']) - 2)

        if random.random() <= self.cr:
            parent1 = Individual(
                net_layers=self.net_layers,
                weights=np.concatenate((parents[0].genes['weights'][:point_w], parents[1].genes['weights'][point_w:])),
                biases=np.concatenate((parents[0].genes['biases'][:point_b], parents[1].genes['biases'][point_b:])),
            )
            parent2 = Individual(
                net_layers=self.net_layers,
                weights=np.concatenate((parents[1].genes['weights'][:point_w], parents[0].genes['weights'][point_w:])),
                biases=np.concatenate((parents[1].genes['biases'][:point_b], parents[0].genes['biases'][point_b:])),
            )
    ...
    def apply_mutation(self, population):
        for individual in population:
            self.mutation(individual, key='weights')
            self.mutation(individual, key='biases')

        return population

    def mutation(self, individual, key='weights'):
        for i in range(len(individual.genes[key])):
            if random.random() <= self.mr:
                individual.genes[key][i] += individual.genes[key][i] * (random.random() - .5) * 3 + (random.random() - 0.5)
